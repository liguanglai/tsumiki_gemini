...
```bash
# フル分析（全要件抽出）
gemini rev-requirements

# 特定の要件カテゴリのみ抽出
gemini rev-requirements --target functional
gemini rev-requirements --target non-functional
gemini rev-requirements --target user-stories

# 信頼度フィルタ
gemini rev-requirements --confidence high
gemini rev-requirements --confidence medium

# 特定のディレクトリを分析
gemini rev-requirements --path ./src

# 出力形式指定
gemini rev-requirements --format markdown,json
```
