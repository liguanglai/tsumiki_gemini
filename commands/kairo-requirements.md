...
2. **プロジェクト基本情報収集**
   - `GEMINI.md` の内容を読み込み（プロジェクト概要・技術スタック・制約）
...
- REQ-301: システムは {オプション機能} してもよい 🔵 *GEMINI.md技術制約より*

### 制約要件

- REQ-401: システムは {制約事項} しなければならない 🔵 *GEMINI.md・既存実装より*

## 非機能要件

### パフォーマンス

- NFR-001: {パフォーマンス要件} 🟡 *GEMINI.md制約から妥当な推測*
...
**NFR-001: {パフォーマンス要件} 🔵 *GEMINI.md制約・ユーザヒアリングより***
...
- "現在のGEMINI.mdで定義されている{制約事項}について、実際の運用で問題ないでしょうか？"
