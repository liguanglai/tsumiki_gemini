...
# 特定のテストカテゴリのみ生成
gemini rev-specs --type unit
gemini rev-specs --type integration
gemini rev-specs --type e2e

# 特定のファイル/ディレクトリを対象
gemini rev-specs --path ./src/auth

# テストコードの実際の生成と出力
gemini rev-specs --generate-code

# カバレッジレポートと合わせて分析
gemini rev-specs --with-coverage

# 優先度フィルタリング
gemini rev-specs --priority high
```
